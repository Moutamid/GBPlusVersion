...
import com.moutamid.gb.databinding.ActivityStatusBinding;
...
public class StattusActivity extends AppCompatActivity implements StatusAdapter.OnCheckboxListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.deleteIV.setOnClickListener(view -> {
            ...
            if (!filesToDelete.isEmpty()) {
                new AlertDialog.Builder(StattusActivity.this)
                        ...
                        .setNegativeButton(getResources().getString(R.string.yes), (dialogInterface, i) -> {
                            ...
                            if (success == 0) {
                                Toast.makeText(StattusActivity.this, getResources().getString(R.string.delete_error), Toast.LENGTH_SHORT).show();
                            } else if (success == 1) {
                                Toast.makeText(StattusActivity.this, getResources().getString(R.string.delete_success), Toast.LENGTH_SHORT).show();
                            }
        ...
        binding.downloadIV.setOnClickListener(view -> {
            ...
            if (!filesToDelete.isEmpty()) {
                ...
                for (StatusModel details : filesToDelete) {
                    File file = new File(details.getFilePath());
                    if (file.exists()) {
                        if (Utils.download(StattusActivity.this, details.getFilePath())) {
                            deletedFiles.add(details);
                            if (success == 0) {
                                return;
                            }
                ...
                myAdapter.notifyDataSetChanged();
                if (success == 0) {
                    Toast.makeText(StattusActivity.this, getResources().getString(R.string.save_error), Toast.LENGTH_SHORT).show();
                } else if (success == 1) {
                    Toast.makeText(StattusActivity.this, getResources().getString(R.string.save_success), Toast.LENGTH_SHORT).show();
                }
                binding.actionLay.setVisibility(View.GONE);
                binding.selectAll.setChecked(false);
            }
    ...
    class loadDataAsync extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            new Handler().postDelayed(() -> {
                myAdapter = new StatusAdapter(StattusActivity.this, f, StattusActivity.this);
                binding.recylerViewImage.setHasFixedSize(true);
                binding.recylerViewImage.setLayoutManager(new GridLayoutManager(StattusActivity.this, 2));
